...
//############################################
Test(bud_malloc_suite, check_diff_pointers,.init = bud_mem_init, .fini = bud_mem_fini) {
    
              
    void *a = bud_malloc(sizeof(int) * 100); 
    void *b = bud_malloc(sizeof(char));      
    void *c = bud_malloc(sizeof(int));      
    void *d = bud_malloc(sizeof(int) * 100); 
    void *e = bud_malloc(sizeof(char));      
    void *f = bud_malloc(sizeof(int));      
    cr_assert((a!=b), "malloc returned one address to two pointer!");
    cr_assert((b!=c), "malloc returned one address to two pointer!");
    cr_assert((c!=d), "malloc returned one address to two pointer!");
    cr_assert((d!=e), "malloc returned one address to two pointer!");
    cr_assert((e!=f), "malloc returned one address to two pointer!");
}
Test(bud_malloc_suite, address_in_range,.init = bud_mem_init, .fini = bud_mem_fini) {
    void *b = bud_malloc(sizeof(char));      
    void *c = bud_malloc(sizeof(int));
    cr_assert((b < bud_heap_end()), "you have allocated a block of memory out of the heap range!");
    cr_assert((c < bud_heap_end()), "you have allocated a block of memory out of the heap range!");
      
    
}
Test(bud_malloc_suite,check_addr_buddy ,.init = bud_mem_init, .fini = bud_mem_fini) {
   
    void *a = bud_malloc(sizeof(long));      
    void *w = bud_malloc(sizeof(int) * 100); 
    void *x = bud_malloc(sizeof(char));      
    void *b = bud_malloc(sizeof(int));      
    void *y = bud_malloc(sizeof(int) * 100); 
    void *z = bud_malloc(sizeof(char));      
    void *c = bud_malloc(sizeof(int));       
    void *d = bud_malloc(sizeof(int));
    bud_header *bhdr = PAYLOAD_TO_HEADER(c); 
    
    int size = ORDER_TO_BLOCK_SIZE(bhdr -> order);
   void* buddyAddress = (void*)((uintptr_t)c ^ (uintptr_t)size);
    cr_assert((d==buddyAddress),"c and d should be buddy");
    cr_assert((z!= buddyAddress), "z and c are not buddy");
    
}
Test(bud_free_suite,check_invalid_hdr ,.init = bud_mem_init, .fini = bud_mem_fini,.timeout = 5, .signal = SIGABRT) {
...
Test(bud_free_suite,check_invalid_ptr ,.init = bud_mem_init, .fini = bud_mem_fini,.timeout = 5, .signal = SIGABRT) {
